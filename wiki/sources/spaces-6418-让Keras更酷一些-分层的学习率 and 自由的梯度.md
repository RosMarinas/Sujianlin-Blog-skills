...
2. **自由的梯度操作**：
   ...
   - **动态覆盖（Monkey Patching）**：利用 Python “处处皆对象”的特点，直接通过将一个自定义的梯度计算函数赋值给已有优化器实例的 `get_gradients` 属性（例如 `adam_opt.get_gradients = our_get_gradients`），即可在模型编译（`compile`）时生效，从而能够实现如 L1 裁剪、特定层梯度操纵或梯度冻结等高度自由的操作。
